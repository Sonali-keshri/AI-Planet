  ...
  const handleSubmit = (e) => {
    ...
    if (!title || !summary || !coverImg || !description || !hackthonName || !startDate || !endDate || !github || !otherLink ) {
      ...
      setArrOfData(
        arrOfData.map((item) => {
          if (item.id === active) {
            return { ...item, timestamp: Date.now(),title,summary,description,coverImg,hackthonName,startDate,endDate,github,otherLink };
          }
          return item;
        })
      );
      ...
      let data = {
          id:v4(), timestamp: Date.now(),title,summary,description,coverImg,hackthonName,startDate,endDate,github,otherLink
        }
